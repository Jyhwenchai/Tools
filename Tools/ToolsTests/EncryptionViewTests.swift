...
struct EncryptionViewTests {
  ...
  @Test("MD5加密UI集成测试")
  func mD5EncryptionUIIntegration() async throws {
  ...
  func base64UIIntegration() async throws {
    let service = EncryptionService.shared
    let input = "Hello World"
  ...
  @Test("AES加密解密UI集成测试")
  func aESUIIntegration() async throws {
    let service = EncryptionService.shared
  ...
  @Test("空输入错误处理测试")
  func emptyInputErrorHandling() async {
    let service = EncryptionService.shared
  ...
  @Test("AES空密钥错误处理测试")
  func aESEmptyKeyErrorHandling() async {
    let service = EncryptionService.shared
  ...
  @Test("Base64解码错误处理测试")
  func base64DecodeErrorHandling() async {
    let service = EncryptionService.shared
  ...
  @Test("哈希算法解密错误处理测试")
  func hashAlgorithmDecryptionError() async throws {
    let service = EncryptionService.shared
  ...
  @Test("算法切换行为测试")
  func algorithmSwitchingBehavior() {
  ...
  @Test("UI输入验证逻辑测试")
  func uIInputValidationLogic() async throws {
    let service = EncryptionService.shared
  ...
  func largeTextProcessingPerformance() async throws {
    let service = EncryptionService.shared
  ...
  @Test("复制功能测试")
  func copyFunctionality() async throws {
    let service = EncryptionService.shared
  ...
  @Test("完整加密流程测试")
  func completeEncryptionFlow() async throws {
    let service = EncryptionService.shared
    ...
    for (input, algorithm, key) in testCases {
      ...
      if algorithm.supportsDecryption {
        ...
        #expect(
          decrypted == input,
          "Decryption should restore original input for \(algorithm.rawValue)")
      }
    }
  }
  ...
  @Test("UI输入输出验证测试")
  func uIInputOutputValidation() async throws {
    let service = EncryptionService.shared
