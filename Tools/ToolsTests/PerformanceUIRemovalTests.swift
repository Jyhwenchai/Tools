...
final class PerformanceUIRemovalTests: XCTestCase {
  ...
  func testPerformanceMonitorOnlyAvailableInDebugMode() {
    ...
    XCTAssertNotNil(performanceMonitor, "Performance monitor should exist but be minimal in RELEASE mode")
  ...
  func testPerformanceUIElementsHiddenInRelease() {
    ...
    XCTAssertTrue(hasPerformanceMonitor, "ContentView should have performance monitor in DEBUG mode")
    ...
    XCTAssertFalse(hasPerformanceMonitor, "ContentView should not have performance monitor in RELEASE mode")
  ...
  func testPerformanceMonitorLoggingBehavior() {
    ...
    XCTAssertTrue(metrics.count >= 0, "Metrics should be available in DEBUG mode")
  ...
  func testPerformanceMonitorMemoryUsage() {
    ...
    #else
    print("✅ RELEASE mode: Basic memory info: \(String(format: "%.1f", report.averageMemoryUsage))MB")
    #endif
  }
  ...
  func testPerformanceMonitorResourceUsage() {
    ...
    #if DEBUG
    print("✅ DEBUG mode: Performance report generated in \(String(format: "%.3f", executionTime))s")
    #else
    print("✅ RELEASE mode: Minimal performance report generated in \(String(format: "%.3f", executionTime))s")
    #endif
  }
}
...
extension PerformanceUIRemovalTests {
  ...
  func testPerformanceMonitorStartupImpact() {
    ...
    XCTAssertLessThan(startupTime, 0.05, "Performance monitor initialization should not slow down startup")
    
    #if DEBUG
    print("✅ DEBUG mode: Performance monitor startup time: \(String(format: "%.3f", startupTime))s")
    #else
    print("✅ RELEASE mode: Minimal performance monitor startup time: \(String(format: "%.3f", startupTime))s")
    #endif
  }
  ...
  func testPerformanceMonitorCleanup() {
    ...
    for _ in 0..<5 {
      let _ = performanceMonitor.getPerformanceReport()
    }
