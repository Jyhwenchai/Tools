...
struct ImageProcessingViewTests {
  @Test("图片处理界面初始化测试")
  func imageProcessingViewInitialization() {
  ...
  func imageSelection() async throws {
    ...
    guard let testImageURL = Bundle.main.url(forResource: "test_image", withExtension: "jpeg")
    else {
  ...
  @Test("图片格式转换测试")
  func imageFormatConversion() async throws {
    let service = ImageProcessingService()
  ...
  @Test("水印添加功能测试")
  func watermarkFunctionality() async throws {
    let service = ImageProcessingService()
  ...
  @Test("图片裁剪功能测试")
  func imageCropping() async throws {
    let service = ImageProcessingService()
  ...
  @Test("处理结果验证测试")
  func processingResultValidation() {
