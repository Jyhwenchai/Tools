...
struct UIIntegrationTests {
  ...
  @Test("导航管理器与主界面集成测试")
  func navigationManagerIntegration() {
  ...
  @Test("工具详情视图集成测试")
  func toolDetailViewIntegration() {
    // 测试每个工具类型都有对应的详情视图
    for toolType in NavigationManager.ToolType.allCases {
  ...
  @Test("共享组件状态管理测试")
  func sharedComponentsStateManagement() {
  ...
  @Test("工具结果视图集成测试")
  func toolResultViewIntegration() {
  ...
  @Test("主题切换集成测试")
  func themeSwitchingIntegration() {
    let settings = AppSettings.shared
  ...
  @Test("性能监控集成测试")
  func performanceMonitoringIntegration() {
  ...
  @Test("异步操作管理集成测试")
  func asyncOperationManagerIntegration() {
  ...
  @Test("安全服务集成测试")
  func securityServiceIntegration() {
  ...
  @Test("粘贴板服务集成测试")
  func clipboardServiceIntegration() {
    // 测试粘贴板项目创建
  ...
  @Test("端到端工作流集成测试")
  func endToEndWorkflowIntegration() {
    // 模拟完整的用户工作流
