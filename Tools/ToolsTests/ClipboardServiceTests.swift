...
struct ClipboardServiceTests {
  ...
  func clipboardItemInitialization() {
  ...
  func clipboardItemTypeDetection(content: String, expectedType: ClipboardItemType) {
    let item = ClipboardItem(content: content)
    #expect(item.type == expectedType)
  }

  @Test("ClipboardItem 预览文本测试")
  func clipboardItemPreview() {
    // Short content
  ...
  func clipboardItemFormattedTimestamp() {
  ...
  func clipboardServiceInitialization() {
...
struct ClipboardItemTypeTests {
  ...
  func clipboardItemTypeRawValues() {
  ...
  func clipboardItemTypeAllCases() {
