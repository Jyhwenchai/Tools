...
struct PerformanceTests {
  ...
  func largeDataProcessingPerformance() async throws {
    // 创建大文本数据 (1MB)
  ...
  func bulkDataCreationPerformance() async throws {
    let itemCount = 10000
    let startTime = Date()
  ...
  @Test("内存使用性能测试")
  func memoryUsagePerformance() async throws {
  ...
  func concurrentOperationsPerformance() async throws {
  ...
  func dataStructurePerformance() async throws {
    // 创建大量粘贴板项目
    let itemCount = 1000
    let startTime = Date()
  ...
  func overallSystemPerformance() async throws {
    let startTime = Date()
