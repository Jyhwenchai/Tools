...
struct PerformanceMonitorTests {
  ...
  func performanceMetricProperties() {
    let timestamp = Date()
  ...
  func performanceWarningDescriptions(warning: PerformanceWarning, expectedDescription: String) {
  ...
  func performanceWarningSeverity(
    warning: PerformanceWarning,
    expectedSeverity: PerformanceWarningSeverity) {
    #expect(warning.severity == expectedSeverity)
  }

  @Test("PerformanceReport 基本属性测试")
  func performanceReportProperties() {
  ...
  @Test("性能监控初始状态测试")
  func performanceMonitorInitialState() {
    let monitor = PerformanceMonitor.shared

    #expect(monitor.currentMemoryUsage >= 0)
    #expect(monitor.currentCPUUsage >= 0)
    #expect(monitor.performanceWarnings.isEmpty || !monitor.performanceWarnings
      .isEmpty) // 可能有也可能没有警告
  }

  @Test("性能报告生成测试")
  func performanceReportGeneration() {
    let monitor = PerformanceMonitor.shared
  ...
  func recentMetricsRetrieval() {
    ...
    if recentMetrics.count > 1 {
      for index in 0..<(recentMetrics.count - 1) {
  ...
  func performanceMonitoringStartStop() async throws {
    ...
    _ = monitor.getPerformanceReport()
  ...
  func releaseModeSimplifiedMonitoring() {
    let monitor = PerformanceMonitor.shared
  ...
  @Test("性能监控无权限要求测试")
  func performanceMonitoringNoPermissionRequirements() {
    let monitor = PerformanceMonitor.shared
