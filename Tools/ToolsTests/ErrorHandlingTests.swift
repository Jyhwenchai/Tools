...
struct ErrorHandlingTests {
  ...
  func toolErrorRecoverySuggestion() {
    let emptyInputError = ToolError.emptyInput
  ...
  @Test("重试服务基本功能测试")
  func retryServiceBasicFunctionality() async throws {
  ...
  @Test("重试服务条件重试测试")
  func retryServiceConditionalRetry() async throws {
    let retryService = RetryService.shared
    var attemptCount = 0
  ...
  @Test("重试服务工具操作测试")
  func retryServiceToolOperation() async throws {
    let retryService = RetryService.shared
    var attemptCount = 0
  ...
  @Test("重试服务非重试错误测试")
  func retryServiceNonRetryableError() async {
    let retryService = RetryService.shared
    var attemptCount = 0
  ...
  @Test("错误恢复建议完整性测试")
  func errorRecoverySuggestionCompleteness() {
