...
struct TimeConverterViewTests {
    ...
    func testTimeConverterViewInitialState() {
        // 测试初始状态的默认值
    ...
    @Test("时区选择器测试")
    func testTimeZonePicker() {
        let commonTimeZones = TimeZoneInfo.commonTimeZones
        
        // 验证包含常用时区
        let identifiers = commonTimeZones.map { $0.identifier }
        ...
        if let utcTimeZone = utcTimeZone {
    ...
    func testInputValidation(input: String, format: TimeFormat, expectedValid: Bool) {
        ...
        if input.isEmpty {
            ...
            let isValid = service.validateDateString(input, format: format, customFormat: "yyyy-MM-dd HH:mm:ss")
    ...
    @Test("时间转换逻辑测试")
    func testTimeConversionLogic() async {
        let service = TimeConverterService()
    ...
    @Test("当前时间格式化测试")
    func testCurrentTimeFormatting() {
        let service = TimeConverterService()
    ...
    func testTimeZoneConversion() {
        ...
        let result = service.convertTime(input: "1640995200", options: utcOptions) // 2022-01-01 00:00:00 UTC
    ...
    func testFormatExamples() {
        ...
        for format in TimeFormat.allCases {
            ...
            #expect(examples.allSatisfy { !$0.isEmpty }, "All examples should be non-empty for \(format.displayName)")
        }
    }
    ...
    func testUIStateManagement() {
        ...
        var outputTime = ""
        var currentError: ToolError? = nil
    ...
    func testPerformance() async {
        ...
        // 测试大量转换操作的性能
        let inputs = Array(1640995200...1640995300).map { String($0) }
        let options = TimeConversionOptions(
            sourceFormat: .timestamp,
            targetFormat: .iso8601,
            sourceTimeZone: TimeZone(identifier: "UTC")!,
...
struct TimeZonePickerTests {
    ...
    func testTimeZonePickerInitialization() {
