...
struct ImageProcessingServiceTests {
    ...
    if !FileManager.default.fileExists(atPath: testImageURL.path) {
      ...
      guard let imageData = testImage.tiffRepresentation,
        ...
        throw NSError(
          domain: "TestError",
          code: 1,
          userInfo: [NSLocalizedDescriptionKey: "Could not create test image data"])
      }
  ...
  @Test("有效URL加载图片测试")
  func loadImageWithValidURL() throws {
  ...
  @Test("有效图片调整大小测试")
  func resizeImageWithValidImage() throws {
    let testImageURL = try getTestImageURL()
  ...
  @Test("保持宽高比调整大小测试")
  func resizeImageWithAspectRatio() throws {
    let testImageURL = try getTestImageURL()
    guard let testImage = service.loadImage(from: testImageURL) else {
      Issue.record("Failed to load test image")
      return
    ...
    let expectedSize = if originalAspectRatio > targetAspectRatio {
      // Image is wider than target
      NSSize(width: targetSize.width, height: targetSize.width / originalAspectRatio)
    } else {
      // Image is taller than target
      NSSize(width: targetSize.height * originalAspectRatio, height: targetSize.height)
    }
  ...
  @Test("有效图片压缩测试")
  func compressImageWithValidImage() throws {
    let testImageURL = try getTestImageURL()
    guard let testImage = service.loadImage(from: testImageURL) else {
      Issue.record("Failed to load test image")
      return
  ...
  @Test("空数组批量处理测试")
  func batchProcessingWithEmptyArray() async {
  ...
  @Test("有效图片批量处理测试")
  func batchProcessingWithValidImage() async throws {
    let testImageURL = try getTestImageURL()
  ...
  @Test("无效URL批量处理测试")
  func batchProcessingWithInvalidURLs() async {
    let invalidURLs = [
  ...
  @Test("图片转JPEG测试")
  func convertImageToJPEG() throws {
    let testImageURL = try getTestImageURL()
    guard let testImage = service.loadImage(from: testImageURL) else {
      Issue.record("Failed to load test image")
      return
  ...
  @Test("不支持格式转换测试")
  func convertImageToUnsupportedFormat() throws {
    let testImageURL = try getTestImageURL()
    guard let testImage = service.loadImage(from: testImageURL) else {
      Issue.record("Failed to load test image")
      return
  ...
  func imageProcessingPerformance() throws {
    let testImageURL = try getTestImageURL()
    guard let testImage = service.loadImage(from: testImageURL) else {
      Issue.record("Failed to load test image")
      return
    ...
    _ = service.resizeImage(
      testImage,
      to: NSSize(width: 500, height: 500),
      maintainAspectRatio: true)
  }

  @Test("图片质量对比测试")
  func imageQualityComparison() throws {
    let testImageURL = try getTestImageURL()
    guard let testImage = service.loadImage(from: testImageURL) else {
      Issue.record("Failed to load test image")
      return
  ...
  func textWatermarkPositions(position: WatermarkPosition) {
    let testImage = createTestImage()
  ...
  @Test("水印位置计算测试")
  func watermarkPositionCalculation() {
  ...
  @Test("批量处理高级功能测试")
  func batchProcessingWithAdvancedFeatures() async throws {
    let testImageURL = try getTestImageURL()
  ...
  func advancedProcessingPerformance() throws {
    let testImageURL = try getTestImageURL()
    guard let testImage = service.loadImage(from: testImageURL) else {
      Issue.record("Failed to load test image")
      return
  ...
  func errorHandling() {
    ...
    service.errorMessage = nil
    let cropResult = service.cropImage(
      invalidImage,
      to: NSRect(x: 0, y: 0, width: 100, height: 100))
