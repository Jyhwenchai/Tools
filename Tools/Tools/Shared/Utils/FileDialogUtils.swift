    ...
    return await withCheckedContinuation { continuation in
      DispatchQueue.main.async {
        ...
        if let message = message {
          ...
          let typeNames = allowedTypes.compactMap { $0.preferredFilenameExtension?.uppercased() }.joined(separator: ", ")
  ...
  static func showEnhancedOpenDialog(
    ...
    return await withCheckedContinuation { continuation in
      DispatchQueue.main.async {
        ...
        let defaultDirectory: FileManager.SearchPathDirectory
        if allowedTypes.contains(.image) {
          ...
          defaultDirectory = .documentDirectory
        } else {
          defaultDirectory = .desktopDirectory
        }
...
struct FileValidationResult {
  ...
  var isValid: Bool {
    return !validFiles.isEmpty && invalidFiles.isEmpty && oversizedFiles.isEmpty
  }
  
  var hasIssues: Bool {
    return !invalidFiles.isEmpty || !oversizedFiles.isEmpty
  }
  ...
  var detailedMessage: String {
    ...
    if !invalidFiles.isEmpty {
      let typeNames = supportedTypes.compactMap { $0.preferredFilenameExtension?.uppercased() }.joined(separator: ", ")
