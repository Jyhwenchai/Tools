...
class PerformanceMonitor {
  ...
  func startPerformanceMonitoring() {
    ...
    DispatchQueue.main.asyncAfter(deadline: .now() + 2.0) { [weak self] in
      ...
      self?.logger.info("🚀 Performance monitoring timer started (delayed for startup optimization)")
    }
    #endif
  }
  ...
  private func logDetailedPerformanceInfo(_ metric: PerformanceMetric) {
    ...
    if performanceMetrics.count >= 3 {
      ...
      if abs(memoryTrend) > 10 {
        print("   📈 Memory trend: \(memoryTrend > 0 ? "+" : "")\(String(format: "%.1f", memoryTrend))MB")
      }
      if abs(cpuTrend) > 5 {
    ...
    logger.info("Memory: \(String(format: "%.1f", metric.memoryUsage))MB, CPU: \(String(format: "%.1f", metric.cpuUsage))%, Optimal: \(self.isPerformanceOptimal)")
  }
  #endif
  ...
  func getPerformanceReport() -> PerformanceReport {
    ...
    let maxMemory = performanceMetrics.map { $0.memoryUsage }.max() ?? currentMemoryUsage
    let maxCPU = performanceMetrics.map { $0.cpuUsage }.max() ?? currentCPUUsage
  ...
  func logPerformanceHistory() {
    ...
    for (index, metric) in performanceMetrics.suffix(10).enumerated() {
      ...
      print("   \(index + 1). [\(time)] Memory: \(String(format: "%.1f", metric.memoryUsage))MB, CPU: \(String(format: "%.1f", metric.cpuUsage))%")
    }
  }
  #endif
}
