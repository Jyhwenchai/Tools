...
class ErrorLoggingService {
  ...
  func logError(_ error: ToolError, context: String? = nil, file: String = #file, function: String = #function, line: Int = #line) {
  ...
  func getErrors(ofType type: String) -> [ErrorLogEntry] {
    return errorHistory.filter { $0.errorType.contains(type) }
  }
  ...
  func getErrorStatistics() -> ErrorStatistics {
    ...
    let retryable = errorHistory.filter { $0.isRetryable }.count
...
struct ErrorStatistics {
  ...
  var mostCommonErrorType: String? {
    return errorTypeCounts.max(by: { $0.value < $1.value })?.key
  }
...
extension View {
  ...
  func withErrorHandling() -> some View {
    self.onAppear {
      ...
      ErrorLoggingService.shared.initialize()
    }
  }
}