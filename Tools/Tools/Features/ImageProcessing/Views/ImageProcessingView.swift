...
struct ImageProcessingView: View {
  ...
  private func openCropTool() {
    guard let firstImageURL = selectedImages.first,
          let image = service.loadImage(from: firstImageURL)
    else {
    ...
    Task {
      ...
      if let folderURL = await FileDialogUtils.showDirectoryDialog(message: message) {
        ...
        for result in successfulResults {
          ...
          let fileName = result.originalURL.deletingPathExtension()
            .lastPathComponent + "_processed." + selectedFormatType.imageFormat.fileExtension
