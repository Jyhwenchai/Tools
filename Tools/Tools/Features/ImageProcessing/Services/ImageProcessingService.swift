...
class ImageProcessingService: ObservableObject {
  ...
  func resizeImage(
    _ image: NSImage,
    to size: NSSize,
    maintainAspectRatio: Bool = true) -> NSImage? {
    let targetSize = maintainAspectRatio ? calculateAspectFitSize(image.size, targetSize: size) :
      size
  ...
  func batchProcess(urls: [URL], operations: [ImageOperation]) async -> [ProcessingResult] {
    ...
    for url in urls {
      ...
      for operation in operations {
        switch operation {
        case let .resize(size, maintainAspect):
          if let resized = resizeImage(
            processedImage,
            to: size,
        ...
        case let .addTextWatermark(text, font, color, opacity, position):
        ...
        case let .addImageWatermark(watermarkImage, opacity, position, scale):
  ...
  func calculateWatermarkRect(
    ...
    case .topRight:
      NSRect(
        ...
        height: textSize.height)
    case .bottomLeft:
      NSRect(x: margin, y: margin, width: textSize.width, height: textSize.height)
    case .bottomRight:
      NSRect(
        x: imageSize.width - textSize.width - margin,
        y: margin,
  ...
  private func convertToHEIC(bitmap _: NSBitmapImageRep) -> Data? {
...
enum ImageFormat: Hashable, CaseIterable {
  ...
  static var allCases: [ImageFormat] {
    [.png, .jpeg(quality: 0.8), .gif, .tiff, .bmp, .webp, .heic]
  }
}
