...
class QRCodeService {
  ...
  func generateQRCode(from text: String, options: QRCodeOptions) throws -> QRCodeGenerationResult {
    ...
    guard let filter = CIFilter(name: "CIQRCodeGenerator") else {
        throw QRCodeError.generationFailed("无法创建QR码生成器")
    }
    filter.setValue(Data(text.utf8), forKey: "inputMessage")
    filter.setValue(options.correctionLevel.coreImageValue, forKey: "inputCorrectionLevel")
  ...
  private func applyColors(to image: CIImage, options: QRCodeOptions) throws -> CIImage {
    ...
    guard let foregroundCIColor = CIColor(color: nsColor),
          let backgroundCIColor = CIColor(color: backgroundNSColor)
    else {
    ...
    guard let colorFilter = CIFilter(name: "CIFalseColor") else {
        throw QRCodeError.generationFailed("无法创建颜色过滤器")
    }
    colorFilter.setValue(image, forKey: "inputImage")
    colorFilter.setValue(backgroundCIColor, forKey: "inputColor0") // 背景色 (0值)
    colorFilter.setValue(foregroundCIColor, forKey: "inputColor1") // 前景色 (1值)
