...
enum TimeFormat: String, CaseIterable, Identifiable {
    ...
    var description: String {
        switch self {
        case .timestamp:
            return "Seconds since January 1, 1970 UTC"
        case .iso8601:
...
struct TimeConversionResult {
    ...
    static func success(result: String, timestamp: TimeInterval? = nil, date: Date? = nil) -> TimeConversionResult {
        TimeConversionResult(success: true, result: result, error: nil, timestamp: timestamp, date: date)
    }
...
struct TimeZoneInfo: Identifiable, Hashable {
    ...
    init(timeZone: TimeZone) {
        ...
        self.offsetFromGMT = timeZone.secondsFromGMT()
    }
