...
class TimeConverterService {
    ...
    func convertTime(input: String, options: TimeConversionOptions) -> TimeConversionResult {
        ...
        guard let sourceDate = parseInput(input, format: options.sourceFormat, timeZone: options.sourceTimeZone, customFormat: options.customFormat) else {
            return .failure(error: "Failed to parse input with format: \(options.sourceFormat.displayName)")
        }
        
        // Then, format the date to target format
        let result = formatDate(sourceDate, format: options.targetFormat, timeZone: options.targetTimeZone, customFormat: options.customFormat, includeMilliseconds: options.includeMilliseconds)
    ...
    func getCurrentTime(format: TimeFormat, timeZone: TimeZone = .current, customFormat: String = "yyyy-MM-dd HH:mm:ss", includeMilliseconds: Bool = false) -> String {
        let currentDate = Date()
        return formatDate(currentDate, format: format, timeZone: timeZone, customFormat: customFormat, includeMilliseconds: includeMilliseconds)
    }
    ...
    func convertTimeZone(date: Date, from sourceTimeZone: TimeZone, to targetTimeZone: TimeZone) -> Date {
    ...
    func validateTimestamp(_ input: String) -> Bool {
        ...
        return timestamp >= 0 && timestamp <= 4102444800 // Jan 1, 2100
    }
    
    func validateDateString(_ input: String, format: TimeFormat, customFormat: String = "") -> Bool {
        return parseInput(input, format: format, timeZone: .current, customFormat: customFormat) != nil
    }
    
    // MARK: - Private Helper Methods
    private func parseInput(_ input: String, format: TimeFormat, timeZone: TimeZone, customFormat: String) -> Date? {
    ...
    private func formatDate(_ date: Date, format: TimeFormat, timeZone: TimeZone, customFormat: String, includeMilliseconds: Bool) -> String {
        switch format {
        case .timestamp:
            let timestamp = date.timeIntervalSince1970
            if includeMilliseconds {
...
extension TimeConverterService {
    ...
    func batchConvert(inputs: [String], options: TimeConversionOptions) -> [TimeConversionResult] {
        return inputs.map { convertTime(input: $0, options: options) }
    }
    
    // MARK: - Time Difference Calculation
    func calculateTimeDifference(from startDate: Date, to endDate: Date) -> (days: Int, hours: Int, minutes: Int, seconds: Int) {
    ...
    func getFormatExamples(for format: TimeFormat, timeZone: TimeZone = .current) -> [String] {
        ...
        return [
            formatDate(sampleDate, format: format, timeZone: timeZone, customFormat: "yyyy-MM-dd HH:mm:ss", includeMilliseconds: false),
            formatDate(pastDate, format: format, timeZone: timeZone, customFormat: "yyyy-MM-dd HH:mm:ss", includeMilliseconds: false),
            formatDate(futureDate, format: format, timeZone: timeZone, customFormat: "yyyy-MM-dd HH:mm:ss", includeMilliseconds: false)
        ]
    }
}